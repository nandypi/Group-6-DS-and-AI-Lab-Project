...
RRF_K = 60  # RRF parameter: score = 1 / (RRF_K + rank)
...
def retrieve_dense(question, top_k=10, timings=None):
    """Search Chroma for dense vector matches.

    Returns top_k results from Chroma. Each result includes document,
...
def retrieve_lexical(question, top_k=10, timings=None):
    """Search BM25 for lexical matches.

    Returns top_k results from BM25 index. Each result includes document,
    metadata, and a BM25 score.
    """
...
def reciprocal_rank_fusion(dense_results, lexical_results, k=RRF_K):
