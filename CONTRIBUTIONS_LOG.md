...
| Team Member | Contribution |
|-------------|--------------|
| **Shubham Gattani** | I assisted Nandan in setting up the VM for our application deployment and configuring the required firewall rules. After the successful deployment, I thoroughly tested the application across both standard and edge-case scenarios and shared my observations and feedback with the team. Finally, I reviewed and verified the implemented guardrails to ensure they were properly configured and functioning as expected. |
| **NandanReddy Parnapalli** | Handled the DevOps and deployment of the application on Google Cloud. Created and configured a Google Cloud VM, set up firewall rules to allow external access to the FastAPI and Streamlit services, and configured a static external IP address for reliable access. Prepared the deployment environment by configuring the required `.env` file, ChromaDB data, and application dependencies, and successfully deployed and ran the application on the cloud VM. |
| **Gurram Sai Sri Ram Hruthik** | Enhanced the RAG pipeline by implementing BM25-based search to improve information retrieval and overall response accuracy. Developed the Streamlit frontend and FastAPI backend to provide an interactive user interface and a scalable backend for deployment. Added a numeric fidelity layer to improve the accuracy and reliability of numerical information in generated responses. Created a dedicated fact database and implemented intelligent re-routing based on whether a user’s question is descriptive or numerical, enabling more appropriate retrieval and response generation. Prepared comprehensive technical, non-technical, and deployment documentation covering the system architecture, implementation, functionality, and deployment process. Also created the final project presentation, incorporating the key aspects of the project, implementation details, results, and overall outcomes. |
| **Shubhashish Biswas** | Assisted with pre-deployment sanity testing and usability testing phases. Provided benchmarks against paid tools' output to queries asked to the application, and drove a comprehensive evaluation on the same parameters as used in RAGAS. Suggested alternatives for deployment and application architecture (esp. front-end) |
| **Akbar Ali** | Developed and tested the Streamlit frontend and FastAPI backend to support an interactive and scalable interface for the RAG system. Implemented and integrated a hybrid retrieval approach combining BM25 lexical search with ENN-based semantic retrieval to improve the relevance and coverage of retrieved information. Conducted experiments and comparative evaluations to assess the effectiveness of the hybrid retrieval strategy, analyzing retrieval quality and overall system performance. Worked on integrating the retrieval pipeline with the application backend and validating the end-to-end flow from user queries through retrieval to generated responses. Also contributed to testing, debugging, and performance analysis of the implemented RAG components to ensure reliable system behavior. |
