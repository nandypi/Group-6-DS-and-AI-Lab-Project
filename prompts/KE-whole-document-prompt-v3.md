...
16. For every category-specific array, follow the exact object structure defined below.

17. Do not add new top-level keys or new keys inside defined objects.

18. Do not omit any top-level key from the output schema.

19. Use `null` for an optional scalar value that is not explicitly stated.

20. Use the exact document section heading when one exists.

21. If no clear section heading exists, return `null` for `section_heading`.

22. Do not invent, shorten, normalize, or paraphrase section headings.

23. Output ONLY valid JSON.
...
* `headcount`: Extract only when headcount is specifically reported as a current result, change, target, restructuring impact, or material workforce update.
...
* avoid duplicating minor variations of the same keyword
* if `is_material` is false, include no more than 8 keywords
